...
## ------------------------------------------------------------------
# Instead of accuracy, What if using Weighted Gini Index ?
# Weighted Gini Index function:
def weighted_gini(y_true, y_pred, weights):
    # Calculate Gini index
    gini = 0
    for c in np.unique(y_true):
        idx = np.where(y_true == c)[0]
        gini += np.sum(weights[idx]) * (1 - np.sum(weights[idx] ** 2))
    return gini

# Define weighted Gini scorer
weighted_gini_scorer = make_scorer(weighted_gini, greater_is_better=False)

# Grid Search with weighted Gini index as scoring metric
param_grid = {'ccp_alpha': np.linspace(0, 0.1, 5),
              'max_depth': [None, 3, 5]
             }
tree_clf = DTC(random_state=0) 
grid_search = GridSearchCV(tree_clf, param_grid, cv=cv, scoring=weighted_gini_scorer)  
grid_search.fit(X, y, sample_weight=np.ones(len(y))) 

best_alpha = grid_search.best_params_['ccp_alpha']  
best_depth = grid_search.best_params_['max_depth'] 
best_score = grid_search.best_score_
print("Best Cost-complexity pruning alpha:", best_alpha) 
print("Best weighted Gini Index:", best_score)
